...
for k = 1:N
    ...
    R1_k =  max(1e-4, interp1(soc_rc, R1_vec, soc_percent, 'linear', 'extrap'));
    R2_k =  max(1e-4, interp1(soc_rc, R2_vec, soc_percent, 'linear', 'extrap'));
    R0_k = max(1e-4, interp1(soc_r0, r0_vals, soc_percent, 'linear', 'extrap'));
    
    % Pour les condensateurs, on les empêche juste d'être trop petits pour éviter la division par zéro
    C1_k = min(10000,max(100,interp1(soc_rc, C1_vec, soc_percent, 'linear', 'extrap')));
    C2_k = min(10000,max(100,interp1(soc_rc, C2_vec, soc_percent, 'linear', 'extrap')));
...
subplot(2,1,1); hold on; grid on;
plot(soc_r0, r0_vals, '-ok', 'LineWidth', 1.5, 'MarkerFaceColor', 'k', 'DisplayName', 'R_0 (Interne)');
plot(soc_rc, R1_vec, '-ob', 'LineWidth', 1.5, 'MarkerFaceColor', 'b', 'DisplayName', 'R_1 (Rapide)');
plot(soc_rc, R2_vec, '-or', 'LineWidth', 1.5, 'MarkerFaceColor', 'r', 'DisplayName', 'R_2 (Lente)');
...
subplot(2,1,2); hold on; grid on;
plot(soc_rc, C1_vec, '-og', 'LineWidth', 1.5, 'MarkerFaceColor', 'g', 'DisplayName', 'C_1 (Rapide)');
plot(soc_rc, C2_vec, '-om', 'LineWidth', 1.5, 'MarkerFaceColor', 'm', 'DisplayName', 'C_2 (Lente)');
xlabel('SoC [%]', 'FontWeight', 'bold');
